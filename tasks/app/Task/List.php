...
<div id="fpo">
	<div id="filter" class="row">
		...
		<div class="wh " wh="done=2" id="asopoasg"> <!-- AND DATEDIFF(deadline_time,NOW())>=0-->
...
<div id="headdd">
	<div class="task">
		...
		<div class="row resp">工作者</div>
		<div class="row resp">稽核人</div>
			...
			with(b){
				...
				html=$(
					'<div class="task p'+priority+'" data-tid="'+id+'">\
						...
						<div class="row resp">'+USERS[resp_id || own_id]+'</div>\
						<div class="row resp">'+(USERS[track] || '無')+'</div>\
						<div class="row title">'+title+'<div class="descript">'+ddd+'</div></div>\
						\
						<div class="row action">\
							<div class="act_icon delete" title="刪除"></div>\
							<div class="act_icon edit" title="編輯" allow=1"  a11ow="'+(allowEdit || fedit?'1':'0')+'"></div>\
...
function bindEvent(){
	...
	$('.task .delete').click(function(e){
		var t=$(this);
		var tid=t.parents('.task').data('tid');
		if(confirm('您確定要刪除此工作?\n\n工作:'+t.parents('.task').find('.title').text())){
			Ajax('Task','delTask',{
				tid:	tid
			},function(){
				ref();
