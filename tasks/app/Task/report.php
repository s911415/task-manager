...
<form class="form">
	<div style="background:#FEFCF5;padding:.75rem 1rem 0;border:1px solid #CCC;margin:.5rem 0">
		...
		<div><pre><?=$info->description?></pre></div>
		<div class="row flex fix">
			...
			<?=Inp('own_id','|建立者','<span style="">'.User::getName($info->own_id).'</span>',false)?>
			<?=Inp('resp_id','|工作人員','<span style="">'.User::getName($info->resp_id).'</span>',false)?>
			<?
			$tun=[];
		...
		<div class="flex row fix">
			...
			echo Inp('deadline_time_s','|完成期限','<span >'.substr($sh,0,16).'</span>',false);
	...
	<div class="flex fix">
		...
		<?=Inp('finish_time','|實際完成日期 <span class="setNow">NOW</span>','<input type="text" class="datetime" value="'.getTime(time(),'html').'"/>',false)?>
	</div>
	<style>
  ...
  border-top: 0;
	*/
	...
	<div class="flex fix">
		...
		echo Inp('progress','|進度','<select class="txt">'.implode('',$adb).'</select>');
		?>
		<script>
		...
		$zx=implode('',$zx);
		echo Inp('spent','|實際花費時間','<select class="txt">'.$zx.'</select>',false);
		?>
		<?
		$zx=[];
		foreach(range(1,24*5) as $zzc){
		...
		$zx=implode('',$zx);
		echo Inp('estimated','|預計花費時間','<select class="txt">'.$zx.'</select>',false);
		?>
	...
	*/
	...
	<div id="detail" class="hide">	
		...
		<?=Inp('deadline_time','期限','<input type="text" class="datetime" value="'.getTime(time()+60*60,'html').'"/>',false)?>
	...
	<div class="row c fix">
		<button class="bt" onclick="window.add_type='report'">回報工作進度</button>
...
addFile();
function fcb(){
	var data=getVal('id|project_id|project_no|show_id|title|own_id|progress|status|description|start_time|end_time|finish_time|priority|score|evaluate|done|deadline_time|spent|estimated');
	getCustField(data);
	addAttach(function(){
...
function addAttach(don){
	var fs=$("#upload input");
	var len=0;
	var count=0;
	var don=don ||function(){
		refFileList();
	};
	fs.each(function(){
...
<script>
var disarr="priority|title|project_id|start_time|deadline_time".split('|').join(',#');
