...
<form class="form">
	...
	*/
	...
	<?=Inp('done','結案','<input type="checkbox"/>',false)?>
	</div>
	...
	<div class="hide">
		...
		<?=Inp('tel_id','tel_id','<input type="hidden" value="-1"/>')?>
		<?=Inp('poster_id','poster_id','<input type="hidden" value="'.Session::get('id').'"/>')?>
		<?=Inp('project_no','project_no','<input type="hidden" value="'.$project_no.'"/>')?>
		<?=Inp('own_id','own_id','<input type="hidden" value="'.Session::get('id').'"/>')?>
	</div>
	<script>
	...
	<div class="row c fix">
		<div class="flex" id="zxcsfdhpoj" style="width:92%;margin:1.25rem auto 0;">
			<button id="oigo" class="bt" onclick="window.add_type='new_new'" >新增本工作並繼續新增</button>
			<button class="" onclick="window.add_type='new'" style="background:#35AEFF;color:#FFF;">新增本工作</button>
...
addFile();
function fcb(){
	var data=getVal('id|project_id|project_no|show_id|title|own_id|resp_id|progress|status|description|start_time|end_time|finish_time|priority|score|evaluate|done|deadline_time|spent|track|tel_id|poster_id');
...
<?
if(isset($_POST['passFromTel'])){
$_POST['passFromTel']['description']=str_replace('\r\n',"\n",$_POST['passFromTel']['description']);
?>
<script>
passVal(<?=json_encode($_POST['passFromTel'])?>);
</script>
<?}?>
<style>
