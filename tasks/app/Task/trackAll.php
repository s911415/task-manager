<div id="userList">
<?
foreach(User::getAllUserList([
	'trackFlag'	=>	1
]) as $u){
?>
	<a href="#" data-uid="<?=$u->id?>" class="user" firstChar="<?=mb_substr($u->name,0,1)?>" title="<?=mb_substr($u->name,1)?>" other="<?=mb_substr($u->name,1)?>"><span><?=$u->name?></span></a>&nbsp;
<?
}
?>
<script>
...
<div id="fpo">
	<div id="filter" class="row">
		<div class="wh sel" wh="DATEDIFF(deadline_time,NOW())<=1 AND done=0"> <!-- AND DATEDIFF(deadline_time,NOW())>=0-->
		...
		<div class="wh" wh="DATEDIFF(deadline_time,NOW())<=3 AND DATEDIFF(deadline_time,NOW())>=0 AND done=0">
		...
		<div class="col hide">
			<label>其他</label>
			<label class="nos">
				<input type="checkbox" value="" id="resp_id" 
				checked
				/>
				只顯示我負責的工作
			</label>
...
<script>
function ref(){
	var Data=getVal('resp_id|track|search');
